...
cacheSolve <- function(x, ...) {   # the input x is an object created by makeCacheMatrix
  m <- x$getinverse()               # accesses the object 'x' and gets the inverted matrix
  ...
  x$setinverse(m)           # store the calculated inverse value in x (see setSolve() in makeVector
...
# cacheSolve(B)
# B$getinverse()
# B$set(matrix(c(1,2,3,4),nrow=2,ncol=2,byrow=TRUE))
# B$get()
# B$getinverse()
# cacheSolve(B)
